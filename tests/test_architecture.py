...
def _layer(path: Path) -> int:
    ...
    if parts[0] == "core" and len(parts) > 1 and parts[1] in (
        "formulas.py", "quant.py", "market_scan.py", "confidence.py", "trade_plan.py", "signal_perf.py", "universe.py", "chart_patterns.py",
    ):
        return 1
    if parts[0] == "db":
        return 2
...
def _module_to_layer(module: str) -> int:
    ...
    if module in (
        ...
        "app.core.trade_plan", "app.core.signal_perf", "app.core.universe", "app.core.chart_patterns",
    ):
        return 1
